    ...
    describe "NewestTopBar"
        ...
        , rspecStyleDescribe "rendering user menus on clicks"
            ...
            , it "shows 'login' when LoggedOut Msg is successful" <|
                NewestTopBar.update (NewestTopBar.LoggedOut (Ok ()))
                    >> Tuple.first
                    >> NewestTopBar.view
                    >> toUnstyled
                    >> Query.fromHtml
        ...
        , rspecStyleDescribe "when triggering a log in message"
            ...
            [ it "redirects to login page when you click login" <|
                Tuple.first
                    ...
                    >> Tuple.second
                    >> Expect.equal (LoginRedirect.requestLoginRedirect "")
            ]
        ...
        , rspecStyleDescribe "when checking search bar values"
            ...
            , it "sends a FilterMsg when the clear search button is clicked" <|
                NewestTopBar.view
                    ...
                    >> Event.simulate (Event.click)
                    >> Event.expect (NewestTopBar.FilterMsg "")
            , it "clears search query when FilterMsg is received with blank" <|
                NewestTopBar.update (NewestTopBar.FilterMsg "")
                    >> Tuple.first
                    >> NewestTopBar.query
                    >> Expect.equal ""
        ...
        , rspecStyleDescribe "when search query is updated"
            ...
            [ it "search item is modified" <|
                NewestTopBar.update (NewestTopBar.FilterMsg "test")
                    >> Tuple.first
                    >> NewestTopBar.query
                    >> Expect.equal ("test")
            ...
            , it "once you pick a status, the dropdown is gone" <|
                NewestTopBar.update NewestTopBar.FocusMsg
                    ...
                    >> NewestTopBar.update (NewestTopBar.FilterMsg "status:")
                    >> Tuple.first
                    >> NewestTopBar.update (NewestTopBar.FilterMsg "status: pending")
                    >> Tuple.first
                    >> NewestTopBar.view
                    >> toUnstyled
                    >> Query.fromHtml
            ...
            [ context "before receiving FocusMsg"
                ...
                , it "sends FocusMsg when focusing on search bar" <|
                    Query.find [ id "search-bar" ]
                        ...
                        >> Event.expect NewestTopBar.FocusMsg
                ]
            , context "after receiving FocusMsg"
                ...
                , it "should trigger a FilterMsg when typing in the search bar" <|
                    Query.find [ id "search-bar" ]
                        ...
                        >> Event.expect (NewestTopBar.FilterMsg "test")
